      ...
      <Wrapper styles={`${layout.wrapper} space-y-8 md:space-y-24`}>
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:gap-10">
          ...
          <div className="flex-1">
              ...
              <ul class="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
                {about.map((item, index) => {
                  return (
                    <li
                      key={item.id}
                      className={`${about.length !== index - 1 ? "mr-2" : ""}`}
                    >
                      <button
                        class={`inline-flex p-4 rounded-t-lg border-b-2 border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 ${
                          index === currentIndex
                            ? "border-b-primary text-primary bg-primary_very_light"
                            : ""
                        }`}
            ...
            <div>
              {currentTab && (
                <Timeline aboutInfoArray={about[currentIndex].info} />
              )}
            </div> */}
